---
title: "11_FDS_ZAM_RBM_Indicators"
...
```{r, eval=TRUE}
##Upload datasets with SDGs (from OneDrive - UNHCR/300 - ST - Survey Team - Main/Survey Programme #Team/Projects/FDS/Countries/Zambia/Data Management/4 Use/With indicators/)
...
People with access to basic facilities are defined as those who have access to all four basic services considered for this indicator: shelter, drinking water, energy and health. Concretely, 1) they live inhabitable housing, which is not overcrowded, provides protection from natural elements and otherthreats, 2)they can reach a primary healthcare facility within one hour from their housing, 3) have electricity for lighting, and 4) use improved sources of drinking water in their housing or within 30minutes round trip collection time. In addition, 5) they live in safe and secure neighbourhoods, proxiedthrough absence of flooding and landmine risks and hazards.
As all five conditions need to be met, values for this indicator can be very low and a detailed analysis ofindicator components is required.
...
| Healthcare     |H00a, H00b, H00c, HH00c1, HH00c2, HH00c3                              |
...
#### Healthcare
Access to healthcare if household has any facility available excluding 'don't know' and 'other' within one hour distance (cannot be > 60) (walking or any other type of transport). 

```{r}
RA_adult <- RA_adult %>%
  mutate(healthcare = ifelse(H00a != 96 & H00a != 98 & H00c == 1, 1, 0)
  ) %>%
...
```{r}
#Add healthcare to the main dataset
main <- main %>%
  left_join(
    RA_adult %>% 
      select(`_uuid`, healthcare),
    by = "_uuid"
  )
...
```{r, eval=TRUE}
#step 6: Combine all services and calculate impact indicator based on electricity, healthcare, drinking water, shelter and landslides_flooding. 

main <-main %>%
  mutate(RBM10202=case_when(
    shelter==0 | electricity==0 | Improved_dw_final==0 | Landslides_flooding==0 | healthcare==0~ 0,
    shelter==1 & electricity==1 & Improved_dw_final==1 & Landslides_flooding==1 | healthcare==1~ 1)
  ) %>% 
...
###Figure for RBM Core Impact 2.2 : Composite Variables by population group
...
composite_impact2_2 <- FDS_ZAM_2025_main %>%
  filter(!is.na(Intro_07)) %>%
  group_by(Intro_07) %>%
  summarise(
    shelter = survey_mean(shelter, na.rm = TRUE),
    electricity = survey_mean(electricity, na.rm = TRUE),
    Improved_dw_final = survey_mean(Improved_dw_final, na.rm = TRUE),
    Landslides_flooding = survey_mean(Landslides_flooding, na.rm = TRUE),
    healthcare = survey_mean(healthcare, na.rm = TRUE)
  ) %>%
  ungroup()
 
composite_impact2_2 <- composite_impact2_2 %>%
  select(Intro_07, shelter, electricity, Improved_dw_final, Landslides_flooding, healthcare) %>%
  pivot_longer(cols = shelter:healthcare, names_to = "facility", values_to = "percentage")
...
composite_impact2_2 <- composite_impact2_2 %>%
  mutate(
    facility_label = dplyr::recode(
      ...
      "healthcare" = "Access to healthcare facility"
    )
  )
...
RA_adult <- RA_adult %>%
  mutate(
    RBM10203 = case_when(
      # Needed and received required care
      H01_1 == 1 & H04 == 1 & H06 %in% c(1, 3) ~ 1,

      # Needed but did not try for reasons that still count as access
      H01_1 == 1 & H04 == 2 &
        (H08_E == 1 | H08_H == 1 | H08_X == 1 | H08_Y == 1) ~ 1,

      # Needed but did not try for other reasons
      H01_1 == 1 & H04 == 2 &
        (H08_A == 1 | H08_B == 1 | H08_C == 1 | H08_D == 1 |
         H08_F == 1 | H08_G == 1) ~ 0,

      # Needed, tried, but could not access
      H01_1 == 1 & H04 == 1 & H06 == 2 ~ 0,

      # Everyone else (did not need or missing) -> NA
      TRUE ~ NA_real_
    ) %>%
      labelled(
        labels = c("Yes" = 1, "No" = 0),
...
# RBM10203 <- FDS_ZAM_2025_RA_adult %>%
...
#     health_access_total = survey_total(RBM10203, na.rm = TRUE), # Total for health_access (numerator)
#     mean_value = survey_mean(RBM10203 / health_all, vartype = c("ci", "se"), na.rm = TRUE)  # Compute mean with NA removed
#   )

#If you are using option 2 to calculate the indicator, use the below code to generate table by population groups: 

RBM10203 <- FDS_ZAM_2025_RA_adult %>%
  filter(!is.na(Intro_07), health_all == 1) %>%   # only those who needed care
  group_by(Intro_07) %>%
  summarise(
    var_name   = "RBM10203",
    num_obs_uw = unweighted(n()),
    denominator = survey_total(),                      # weighted count of “needed care”
    mean_value  = survey_mean(RBM10203, vartype = c("ci","se"), na.rm = TRUE)
  )
...
RBM10203_AGD <- FDS_ZAM_2025_RA_adult %>%
  filter(!is.na(HH_02_RA), 
         !is.na(disability_RA), 
         !is.na(Intro_07)) %>%
  group_by(Intro_07, HH_02_RA, disability_RA) %>%
  summarise(
    var_name    = "RBM10203",                  
    num_obs_uw  = unweighted(n()),             # unweighted N
    denominator = survey_total(),              # weighted denominator
    mean_value  = survey_mean(RBM10203, vartype = c("ci", "se"), na.rm = TRUE), 
    .groups = "drop"
  )
...
This indicator measures the primary education gross enrolment rate. It considers all children and young people, regardless of their age, who are officially registered in primary education programmes in anytype of primary school and education institution, expressed as a percentage of the official total primary-school aged population.
...
HHroster <- HHroster %>%
  mutate(edu_primary = case_when(
    HH_Educ02a == 1 & HH_Educ03 %in% 1:7 ~ 1, # Ensure to check possible grade levels. In these example answers 1 - 7 were considered for primary grade level. 
...
HHroster <- HHroster %>%
  mutate(
    age_primary = case_when(
      ...
      agetouse >= 7 & agetouse <= 14 ~ 1,  #Remember to adjust age levels based on the primary education age per country

      HH_04 >= 7 & HH_04 <= 14 ~ 1,  #Remember to adjust age levels based on the primary education age per country
      TRUE ~ 0
    )
  )
...
HHroster <- HHroster %>%
  mutate(
    rbm10302a = ifelse(age_primary == 1, edu_primary, NA_real_)
  )
```
...
RBM10302A <- FDS_ZAM_2025_HHroster %>%
  filter(!is.na(Intro_07)) %>%
  group_by(Intro_07) %>%
  summarise(
    var_name    = "impact3_2a",
    num_obs_uw  = unweighted(n()),
    denominator = survey_total(!is.na(rbm10302a), na.rm = TRUE),
    mean_value  = survey_mean(rbm10302a, vartype = c("ci", "se"), na.rm = TRUE),
    .groups = "drop"
  )
...
RBM10302A_AGD <- FDS_ZAM_2025_HHroster %>%
  filter(!is.na(Intro_07), 
         !is.na(disability), 
         !is.na(HH_02)) %>%
  group_by(Intro_07, disability, HH_02) %>%
  summarise(
    var_name    = "impact3_2a",
    num_obs_uw  = unweighted(n()),
    denominator = survey_total(!is.na(rbm10302a), na.rm = TRUE),
...
This indicator measures the secondary education gross enrolment rate. It considers all children andyoung people, regardless of their age, who are officially registered in secondary education programmesin any type of secondary school and education institution, expressed as a percentage of the official totalsecondary-school aged population.

"Enrollment" refers to individuals officially registered in a secondary school education programe.
...
HHroster <- HHroster %>%
  mutate(edu_secondary = case_when(
    HH_Educ02a == 1 & HH_Educ03 %in% 8:12 ~ 1, # Ensure to check possible grade levels in the country specific questionnaire. In this example answers 1 - 8 were considered for secondary grade level. 
    HH_Educ02a %in% c(99, 98) ~ NA_real_, #When respondent answers with 99 - refuse to answer, or 98 - Don't know this is treated as NA. 
    HH_Educ02a == 2 ~ 0
  ))
...
HHroster <- HHroster %>%
  mutate(
    age_secondary = case_when(
      agetouse >= 14 & agetouse <= 19 ~ 1,  #Remember to adjust age levels based on the secondary education age per country
      TRUE ~ 0
    )
  )
```
...
# Define indicator at roster level first
HHroster <- HHroster %>%
  mutate(
    rbm10302b = ifelse(age_secondary == 1, edu_secondary, NA_real_)
  )

# Redefine survey design object
FDS_ZAM_2025_HHroster <- HHroster %>%
  as_survey_design(
    strata=samp_strat,
...
# Summarise by population group
RBM10302B <- FDS_ZAM_2025_HHroster %>%
  filter(!is.na(Intro_07)) %>%
  group_by(Intro_07) %>%
  summarise(
    var_name    = "impact3_2b",
    num_obs_uw  = unweighted(n()),                        # unweighted N
    denominator = survey_total(!is.na(rbm10302b), na.rm = TRUE), # weighted denominator
    mean_value  = survey_mean(rbm10302b, vartype = c("ci", "se"), na.rm = TRUE),
    .groups = "drop"
  )
...
RBM10302B_AGD <- FDS_ZAM_2025_HHroster %>%
  filter(!is.na(Intro_07), !is.na(disability), !is.na(HH_02)) %>%
  group_by(Intro_07, disability, HH_02) %>%
  summarise(
    var_name     = "impact3_2b",
    num_obs_uw   = unweighted(n()),
    denominator  = survey_total(!is.na(rbm10302b), na.rm = TRUE),          # weighted eligible
    mean_value   = survey_mean(rbm10302b, vartype = c("ci","se"), na.rm = TRUE),  # proportion (0–1)
    .groups = "drop"
  )
...
People feeling safe walking alone in their neighbourhood after dark are defined as those who respondthey feel ‘safe’ or ‘very safe’ in the survey.
Note this indicator only pertains to the self-reported feeling of ‘safety’ which is subjective andcontextual. It does not relate to ‘security’ which is associated with additional external factors.
...
RBM10303 <- FDS_ZAM_2025_RA_adult %>%
  filter(!is.na(Intro_07)) %>%                     # Exclude if pop groups is NA
...
RBM10303_AGD <- FDS_ZAM_2025_RA_adult %>%
...
*Indicator Code*:strata=samp_strat

Children under the age of five years whose births have been registered are defined as those whose birthwas reported as registered with civil authorities at the time of the survey or in possession of a birthcertificate, regardless of whether this was shown to the interviewer. Survey data should be triangulatedwith other sources including national state civil registries and/or refugee/IDP registration data asavailable.
...
RBM20102 <- FDS_ZAM_2025_HHroster %>%
  filter(!is.na(Intro_07)) %>%         # Exclude if pop groups is NA
  group_by(Intro_07) %>%                           # Group by pop_groups
  summarise(                                         # Summarise to compute values
...
RBM20102_AGD <- FDS_ZAM_2025_HHroster %>%
...
HHroster <- HHroster %>%
  mutate(
    RBM20103 = case_when(
      # 1. Age >=15 AND has any recognised ID or document → 1
      agetouse >= 15 &
      (
        ID_09 %in% c("H","I","II","M","N") |
        ID_17a == 1 |
        ID_17b == 1 |
        ID_17c == 1 |
        ID_17d == 1 |
        ...
        Legal_asylum_cert       == 1 |
        Legal_alien_card        == 1 |
        Legal_registration_cert == 1 |
        Legal_birth_cert        == 1 |
        Legal_passport          == 1
      ) ~ 1,

      # 2. Refugees / Former Refugees, age >=15, no recognised IDs AND none of the docs ==1 → 1
      agetouse >= 15 &
      Intro_07 %in% c("Refugees","Former Refugees") &
      !ID_09 %in% c("H","I","II","M","N") &
        ID_17a != 1 |
        ID_17b != 1 |
        ID_17c != 1 |
      ...
      # 3. Host community, age >=15, no birth cert and no passport → 0
      agetouse >= 15 &
      Intro_07 == "Host Community" &
      Legal_birth_cert != 1 &
      Legal_passport   != 1 ~ 0,
...
RBM20103 <- FDS_ZAM_2025_HHroster %>%
  filter(!is.na(Intro_07)) %>%         # Exclude if pop groups is NA
  group_by(Intro_07) %>%                           # Group by pop_groups
  summarise(                                         # Summarise to compute values
...
RBM20103_AGD <- FDS_ZAM_2025_HHroster %>%
...
People who know where to access available GBV services are defined as those who self-reportknowledge of at least one of the two available lifesaving gender-based violence services (Health orPsychosocial/Case Management services)
in the aftermath of a GBV incident.

The indicator survey questions collects information on knowledge of four GBV response services, but  the calculation itself focuses only on knowledge of two lifesaving GBV services that should be immediately set up at the onset of an emergency, health and psycho social services. 
...
RBM20401 <- FDS_ZAM_2025_RA_adult %>%
  filter(!is.na(Intro_07)) %>%                     # Exclude if pop groups is NA
  group_by(Intro_07) %>%                           # Group by pop_groups
  summarise(                                         # Summarise to compute values
...
RBM20401_AGD <- FDS_ZAM_2025_RA_adult %>%
  filter(!is.na(HH_02_RA) & !is.na(disability_RA) & !is.na(Intro_07) ) %>%  # Exclude HH07_cat categories 1, 2, and 5
  group_by(Intro_07, HH_02_RA,disability_RA) %>%
  summarise(
    var_name = "outcome4_1",                # Name of the variable
...
People who do not accept violence against women are defined as those who respond ‘no’ to all of the fivestandard questions, i.e., believe that a husband is not justified in hitting or beating his wife for any of thefive listed reasons: 1) Going out without telling him; 2) Neglecting the children; 3) Arguing with him; 4)Refusing to have sexual intercourse with him; 5) Burning food.
...
# 
# 
# RBM20402 <- FDS_ZAM_2025_RA_adult %>%
#   filter(!is.na(Intro_07)) %>%                     # Exclude if pop groups is NA
#   group_by(Intro_07) %>%                           # Group by pop_groups
#   summarise(                                         # Summarise to compute values
...
# RBM20402_AGD <- FDS_ZAM_2025_RA_adult %>%
#   filter(!is.na(HH_02_RA) & !is.na(disability_RA) & !is.na(Intro_07) ) %>%  # Exclude HH07_cat categories 1, 2, and 5
#   group_by(Intro_07, HH_02_RA,disability_RA) %>%
#   summarise(
#     var_name = "outcome4_2",                # Name of the variable
...
People who have energy to ensure lighting are defined as those in households that use one of thefollowing as light source most of the time: electricity (including solar panels), solar home systems, solar-powered lantern or flashlight, rechargeable flashlight, mobile, torch or lantern, battery poweredflashlight, torch or lantern, biogas lamp, LPG lamp..
...
Children who have received measles vaccination are defined as those aged between 9 months to fiveyears who have received at least one dose of a measles containing vaccine (measles or MMR - MeaslesMumps and Rubella).
...
RBM21001 <- FDS_ZAM_2025_RA_caregiver %>%
  filter(!is.na(Intro_07)) %>%                     # Exclude if pop groups is NA
  group_by(Intro_07) %>%                           # Group by pop_groups
  summarise(                                         # Summarise to compute values
...
RBM21001_AGD <- FDS_ZAM_2025_RA_caregiver %>%
...
Births attended by skilled health personnel are defined as those, where women aged 15-49, who had alive birth in the past two years, were assisted during delivery by skilled health professionals: doctors,nurses or midwives. The place of latest live childbirth is also recorded.
...
RBM21002  <- FDS_ZAM_2025_RA_woman %>%
  filter(!is.na(Intro_07)) %>%                     # Exclude if pop groups is NA
  group_by(Intro_07) %>%                           # Group by pop_groups
  summarise(                                         # Summarise to compute values
...
This indicator is defined as the percentage of people using at least basic water services. People using at least basic drinking water services are defined as those in households using improvedsources of drinking water either in their dwelling/yard/plot or within 30 minutes round trip collectiontime, including queuing. 
...
This indicator measures the proportion of people with access to at least basic sanitation services. People with access to a safe household toilet are defined as those in households with improvedsanitation facilities that are emptied safely and that are not shared with other households.
...
People with an accountat a bank or other financial institution or with a mobile-money-service provider
are defined as those who report having an account (by themselves or together with someone else) at a bank or another type of financial institution or personally using a mobile money service in the past 12 months in the country of asylum or habitual residence (for returnees, countries of origin are included).
...
RBM21301 <- FDS_ZAM_2025_RA_adult %>%
  filter(!is.na(Intro_07)) %>%                     # Exclude if pop groups is NA
  group_by(Intro_07) %>%                           # Group by pop_groups
  summarise(                                         # Summarise to compute values
...
RBM21301_AGD <- FDS_ZAM_2025_RA_adult %>%
  filter(!is.na(HH_02_RA) & !is.na(disability_RA) & !is.na(Intro_07) ) %>%  # Exclude HH07_cat categories 1, 2, and 5
  group_by(Intro_07, HH_02_RA,disability_RA) %>%
  summarise(
    var_name = "outcome13_1",                # Name of the variable
...
People who self-report positive changes in their income compared to previous year are defined as thosewho state their income increased and who can also afford more goods and services, or those whoseincome remained the same or decreased but who can still afford more goods and services (to account forpossible inflation).
...
This indicator measures the level of unemployment among forcibly displaced and stateless people. People who are unemployed are defined as those individuals in the labour force who were not inemployment and seeking a job within 7 days prior to the reference date and who are available to startwork within the next 14 days.
Note that individuals who worked for pay or profit, even if only for one hour, in past 7 days, or who weretemporarily absent from a job to which they will return are considered employed.
...
RBM21303 <- FDS_ZAM_2025_RA_adult %>%
  filter(!is.na(Intro_07)) %>%                     # Exclude if pop groups is NA
  group_by(Intro_07) %>%                           # Group by pop_groups
  summarise(                                         # Summarise to compute values
...
RBM21303_AGD <- FDS_ZAM_2025_RA_adult %>%
  filter(!is.na(HH_02_RA) & !is.na(disability_RA) & !is.na(Intro_07) ) %>%  # Exclude HH07_cat categories 1, 2, and 5
  group_by(Intro_07, HH_02_RA,disability_RA) %>%
  summarise(
    var_name = "outcome13_3",                # Name of the variable
...
Refugee returnees with legally recognized identity documents or credentials are defined as those who report to be in possession of valid documents that are legally recognised by national law and which may be used asproof of identity, to establish nationality or legal status and associated rights vis-à-vis the host State and/or UNHCR (including birth certificates, passports national or refugee IDs amongst others),regardless of whether these were shown to the interviewer.
...
# ###Table 
# 
# RBM21401 <- FDS_ZAM_2025_HHroster %>%
...
table(main$Plot1_Land05) #Do you or does any member of your household have ownership rights for this plot?
table(main$Plot1_Land07) #Do you or does any member of your household have use rights for this plot?
table(main$Plot1_Land09) #Residential use of this plot
table(main$Plot1_Land10) #Does your household have a document for this plot from any government Agency, which formally recognizes the use/ownership rights?
table(main$Plot1_Land15) #How likely is it that someone would take this plot from your household in the next 5 years? Very unlikely, somewhat unlikely, somewhat likely, or very likely?
...
main <- main %>%
  mutate(residental_plot = case_when(
    grepl("A", Plot1_Land09) ~ 1,#Assigns 1 if the use of plot 1 is residential
    !grepl("A", Plot1_Land09) ~ 0)) #Assigns 0 if the use of plot 1 is not residential
```
...
main <- main %>%
  mutate(use_own_current_plot = case_when(
    Plot1_Land05 == 1 | Plot1_Land07 == 1 ~ 1, #Assigns 1 if the HH has ownership or use rights of the plot they live on
    Plot1_Land05 == 2 & Plot1_Land07 == 2 ~ 0,  #Assigns 0 if the HH does not have ownership nor use rights of the plot they live on
    Plot1_Land05 %in% c(98,99) & Plot1_Land07 == 2 ~ 0, #Assigns 0 if the HH does not have use rights of the plot they live on & they don't know/refuse      to answer to ownership rights
    TRUE ~ NA_real_ )) #Assigns NA if the answer is "don't know" or "refuse to answer".
```
```{r, eval=TRUE}
...
main <- main %>%
  mutate(legal_documents = case_when(
    Plot1_Land10 == 1 ~ 1, #Assigns 0 if the HH has legal ownership or use right documents for the plot they live on
    Plot1_Land10 == 2 ~ 0, #Assigns 0 if the HH does not have legal ownership or use right documents for the plot they live on
    TRUE ~ NA_real_ )) #Assigns NA if the answer is "don't know" or "refuse to answer".
```
...
main <- main %>%
  mutate(plot_security = case_when(
    Plot1_Land15 %in% c(1,2) ~ 1, #Assigns 1 when the likelihood is "Very unlikely" and "Somewhat unlikely"
    Plot1_Land15 %in% c(3,4) ~ 0)) #Assigns 0 when the likelihood is "Somewhat likely" and "Very likely"
...
Types of social protection benefits and services are recorded and categorized under: social assistance,social insurance, social services or labour market policies.
...
main <- main %>%
  rowwise() %>%
  mutate(
    Social_Benefit = case_when(
      any(c_across(starts_with("ScProtec16") & !matches("ScProtec16atrigger")) == 1, na.rm = TRUE) ~ 1,
      all(c_across(starts_with("ScProtec16") & !matches("ScProtec16atrigger")) == 2, na.rm = TRUE) ~ 0,
      TRUE ~ 0
    )
  ) %>%
  ungroup()
...
programme_columns <- grep("^ScProtec04\\d+$", names(main), value = TRUE)

```
```{r, eval=TRUE}
#Step 2.3: Create a new varibale checking if the household received any assistance programme from the government

# Create Government assistance variable
main <- main %>%
  mutate(
    Government_assistance = case_when(
      ScProtec01 == 1 & apply(main[programme_columns], 1, function(row) any(row == 1, na.rm = TRUE)) ~ 1, # yes, government assistance
      ScProtec01 == 2 ~ 0,  # no assistance
      ScProtec01 == 1 & apply(main[programme_columns], 1, function(row) all(row != 1, na.rm = TRUE)) ~ 0, # assistance but not from govt
      TRUE ~ NA_real_  # default = NA (safer than forcing 0)
    )
  )
...
write_xlsx(combined_RBM_indicators, "Combined_Indicators_RBM.xlsx")
...
write_xlsx(combined_RBM_indicators_AGD, "Combined_Indicators_RBM_AGD.xlsx")
